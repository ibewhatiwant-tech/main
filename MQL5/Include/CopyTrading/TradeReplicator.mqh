   ...
   if(ok)
     {
      ...
      if(m_perf != NULL)
         m_perf.RecordTradeCopied(
            (signal.price > 0.0 && SymbolInfoDouble(signal.symbol, SYMBOL_POINT) > 0.0)
            ? MathAbs(price - signal.price) / SymbolInfoDouble(signal.symbol, SYMBOL_POINT) / 10.0
            : 0.0);
     }
   else
     {
